...
/*
   ...
   Modifications from Scil (contact@scil.coop), copyright 2012:
     Support for exponential numbers in parseNumbers
     Support for arcs in drawArc
     Support for g transforms
     Support for gradient strokes int doStroke
 */
...
//        Util.debug("Parsing complete in " + (System.currentTimeMillis() - start) + " millis.");
            ...
            switch (c) {
                ...
                case '-': {
                    ...
                    if (str.trim().length() > 0) {
                        //Util.debug("  Next: " + str);
                        if (c == '-' && s.charAt(i-1) == 'e') {
                            // This is not the end of a number, continue
                            continue;
                        }
                        Float f = Float.parseFloat(str);
                        numbers.add(f);
    ...
    private static void drawArc(Path p, float lastX, float lastY, float x, float y, float rx, float ry, float theta, int largeArc, int sweepArc) {
        ...
        ry = (ry < 0 ? -ry : ry);
        theta = theta % 360;
        boolean bLargeArc = largeArc != 0;
        boolean bSweepArc = sweepArc != 0;
        
        // Let's get serious: see paragraph #F.6.5 of the implementation notes
        // for math explanations
        // Step 1
        double xPrime = Math.cos(theta) * ((lastX - x) / 2.0)
                        + Math.sin(theta) * ((lastY - y) / 2.0);
        double yPrime = Math.cos(theta) * ((lastY - y) / 2.0)
                        - Math.sin(theta) * ((lastX - x) / 2.0);
        // Step 2
        double cConst = Math.sqrt( (rx*rx * ry*ry - rx*rx * yPrime*yPrime - ry*ry * xPrime*xPrime)
                                  / (rx*rx * yPrime*yPrime + ry*ry * xPrime*xPrime));
        double cxPrime = cConst * (rx * yPrime / ry);
        double cyPrime = cConst * (-1 * (ry * xPrime / rx));
        boolean positiveC = bLargeArc != bSweepArc;
        cxPrime = (positiveC ? cxPrime : -cxPrime);
        cyPrime = (positiveC ? cyPrime : -cyPrime);
        // Step 3
        double cx = Math.cos(theta) * cxPrime - Math.sin(theta) * cyPrime
                   + (lastX + x) / 2.0;
        double cy = Math.sin(theta) * cxPrime + Math.cos(theta) * cyPrime
                   + (lastY + y) / 2.0;
        // Step 4
        double vx = (xPrime - cxPrime) / rx;
        double vy = (yPrime - cyPrime) / ry;
        double wx = (-xPrime - cxPrime) / rx;
        double wy = (-yPrime - cyPrime) / ry;
        double phy1 = Math.acos( ( vx )
                                / 1.0 * Math.sqrt(vx*vx + vy*vy) );
        phy1 = (vy > 0 ? phy1 : -phy1);
        double deltaPhy = Math.acos( ( vx * wx + vy * wy)
                                    / Math.sqrt(vx*vx + vy*vy)
                                      * Math.sqrt(wx*wx + wy*wy) );
        deltaPhy = (vx * wy - vy * wx > 0 ? deltaPhy : -deltaPhy);
 
        RectF oval = new RectF((float)(cx - rx), (float)(cy - ry),
                               (float)(cx + rx), (float)(cy + ry));
        p.addArc(oval, (float)phy1 * 180, (float)deltaPhy * 180);
    }
...
//            Log.d(TAG, "Float parsing '" + name + "=" + v + "'");
    ...
    private static class SVGHandler extends DefaultHandler {
        ...
        private boolean doStroke(Properties atts) {
            ...
            Integer color = null;
            if (strokeString != null && strokeString.startsWith("url(#")) {
                // It's a gradient fill, look it up in our map
                String id = strokeString.substring("url(#".length(), strokeString.length() - 1);
                Shader shader = gradientMap.get(id);
                if (shader != null) {
                    //Util.debug("Found shader!");
                    paint.setShader(shader);
                    paint.setStyle(Paint.Style.FILL);
        ...
        public void startElement(String namespaceURI, String localName, String qName, Attributes atts) throws SAXException {
            ...
            if (localName.equals("svg")) {
                ...
                if (gradient != null) {
                    ...
                    if (colorStyle != null) {
                        color = SVGColors.colorValue(colorStyle);
                    }
                ...
                // Get it's transformation matrix if any
                String transform = getStringAttr("transform", atts);
                if (transform != null) {
                    final Matrix matrix = parseTransform(transform);
                    groupMatrixes.add(matrix);
                    canvas.save();
                    canvas.concat(matrix);
                } else {
                    // Add identity to make pop secure
